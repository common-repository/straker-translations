...
(function ($, window) {
	...
	Jconfirm.prototype = {
		...
		_bindEvents: function () {
			...
			this.$scrollPane.click(
				function (e) { // Ignore propagated clicks
		...
		_imagesLoaded: function () {
			...
			$.each(
				this.$content.find( 'img:not(.loaded)' ),
				function (i, a) {
					that.imageLoadInterval = setInterval(
						function () {
							var h = $( a ).css( 'height' );
							if (h !== '0px') {
								$( a ).addClass( 'loaded' );
		...
		_setButtons: function () {
			...
			$.each(
				...
				function (key, button) {
					...
					$.each(
						that.buttons[key].keys,
						function (i, a) {
							that.buttons[key].keys[i] = a.toLowerCase();
						}
					);

					var button_element            = $( '<button type="button" class="btn ' + that.buttons[key].btnClass + '">' + that.buttons[key].text + '</button>' ).click(
						function (e) {
							e.preventDefault();
